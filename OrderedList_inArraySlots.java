...
public class OrderedList_inArraySlots
    ...
    public int indexOf( Integer findMe) {
        //return binarySearchR(0, this.size() - 1, findMe);
		return binarySearchW(0, this.size() - 1, findMe);
    }
	
	public int binarySearchR(int low, int hi, int target){
		if(low > hi) return -1;
		int mid = ((hi + low)/2);
		if (list_iAS.get(mid) == target) return mid;
		if (list_iAS.get(mid) > target) return binarySearchR(low, mid - 1, target);
		else return binarySearchR(mid + 1, hi, target);
	}
	
	public int binarySearchW(int low, int hi, int target){
		while (low <= hi){
		int mid = ((hi + low)/2);
		if (list_iAS.get(mid) == target) return mid;
		if (list_iAS.get(mid) > target) hi = mid - 1;
		else low = mid + 1;
		}
		return -1;
	}
